...
let verifyLoggedIn = async () => {
	let res = await fetch('http://localhost:8080/RESProject1/getSession');
...
document.getElementById("logout").addEventListener('click', async () => {
	let res = await fetch('http://localhost:8080/RESProject1/logout');
	userId = -1;
	verifyLoggedIn();
});
...
document.getElementById("filter").addEventListener('click', async () => {
	...
	console.log(status);
	if(status<3){
		let res = await fetch(`http://localhost:8080/RESProject1/filterReimbursements?status=${status}`);
		let obj = await res.json();
		populateTable(obj);
	}
	else{
...
let retreiveAllReimbursements = async () => {
	let res = await fetch(`http://localhost:8080/RESProject1/getAllReimbursements`);
	let obj = await res.json();
	return obj;
}
...
let init = async () => {
	...
	let res = await fetch(`http://localhost:8080/RESProject1/getUser?userid=${userId}`);
