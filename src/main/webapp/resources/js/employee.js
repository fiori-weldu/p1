...
document.getElementById("logout").addEventListener('click', async () => {
	let res = await fetch('http://localhost:8080/RESProject1/doLogout');
	userId = -1;
	verifyLoggedIn();
});
...
let retreiveAllReimbursements = async () => {
	let res = await fetch(`http://localhost:8080/RESProject1/getAllById?id=${userId}`);
	let obj = await res.json();
...
let submitTicket = async (e) => {
	...
	if (!amount || !date || !desc) {
		...
		let req = await fetch('http://localhost:8080/RESProject1/addNewReimbursement', {
			method: 'POST',
			headers: {
